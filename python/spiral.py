...
def qwerty(t, phi, ksi, H, n):
    plt.subplot(4, 2, n)  # 3-строки, 1 столбец, 1 индекс
    plt.plot(ksi, phi)
    plt.subplot(4, 2, n+1)  # 3-строки, 1 столбец, 1 индекс
    plt.plot(t, H)
...
if __name__ == '__main__':
    ...
    t, phi, ksi = np.loadtxt("../data/basic_kah/data"+str(N)+".csv", unpack=True)
    H = (np.multiply(ksi, ksi) + 4 * np.multiply(phi, phi))/400 - 1
